...
export default function UpdatePostScreen() {
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        setDescription((post as any).description || '');
  ...
  const handleUpdate = async () => {
    ...
    if (images.length === 0) {
      Alert.alert('Missing Photos', 'Please keep at least one photo.');
      return;
    }
  ...
  if (isFetching) {
    return (
      <View className="flex-1 bg-[#FAFAFA] items-center justify-center">
        ...
        <Text className="mt-4 text-[#8A8F87]">Loading your listing...</Text>
      </View>
    );
  }
          ...
          >
            {isPublishing ? (
              ...
              <>
                ...
                <Text className="text-white text-[16px] font-semibold ml-2">Update Listing</Text>
              </>
            )}
          </TouchableOpacity>
        </View>
