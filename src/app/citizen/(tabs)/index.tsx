    ...
    <SafeAreaView className="flex-1 bg-[#FAFAFA]" style={{ paddingTop: Platform.OS === 'android' ? 40 : 0 }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="px-6 pt-4 pb-5">
          <Text className="text-[28px] font-bold text-[#1E5631] tracking-tight">Browse Waste</Text>
        </View>
            ...
            {CATEGORIES.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  <Text className={`text-[13px] font-semibold ${isActive ? 'text-[#1E5631]' : 'text-[#8A8F87]'}`}>
                    {cat.name}
                  </Text>
                </TouchableOpacity>
              );
        ...
        <View className="px-6 flex-row flex-wrap justify-between">
          {isLoading ? (
            ...
            <View className="w-full items-center justify-center py-10">
              <Text className="text-[#8A8F87] text-[15px]">No items found in this category.</Text>
            </View>
          ) : (
            posts.map((item) => {
              // Fallback image if the post has no images attached
              ...
              return (
                <TouchableOpacity 
                  key={item.id} 
                  activeOpacity={0.8}
                  onPress={() => router.push(`/citizen/${item.id}`)} // 3. Dynamic navigation link added here
                ...
                >
                  ...
                  <View className="w-full h-36 rounded-[18px] overflow-hidden mb-3 relative bg-[#F9F9F9]">
                    ...
                    {/* Price/Free Badge */}
                    <View 
                      className={`absolute top-2 right-2 px-2.5 py-1 rounded-md 
                      ${isFree ? 'bg-[#2ECC71]' : 'bg-[#F1C40F]'}`}
                    >
                      <Text className="text-white text-[10px] font-bold tracking-wider">{badgeText}</Text>
                    </View>

                    {/* Category Icon Badge */}
                    <View className="absolute bottom-2 left-2 w-8 h-8 bg-white rounded-full items-center justify-center shadow-sm">
                      <MaterialCommunityIcons name={getCategoryIcon(item.category) as any} size={16} color="#1E5631" />
                    </View>
                  </View>

                  {/* Text Info */}
                  <View className="px-1 pb-1">
                    <Text className="text-[14px] font-semibold text-[#1b1c1c] mb-1.5" numberOfLines={1}>
                      {(item as any).title || 'Untitled Material'}
                    </Text>
                    <View className="flex-row justify-between items-center">
                      <Text className="text-[12px] text-[#3d4a3f] font-medium">{weight}</Text>
                      <Text className="text-[11px] text-[#8A8F87]">{formattedDate}</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              )
